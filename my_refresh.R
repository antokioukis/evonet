...
create_sim<-function(swapp,rr,N,name){
	...
	foreach(i = 1:n) %dopar% {
	    ...
	    print("meta")
	    cmd <- paste(curDir, "/evonet -tarfit 0.95 -key_genes 10 -selection ", sel," -st_geno R1R2_input.txt -s2 ", s, " -N ", N," -ploidy ", ploidy, " -swapping ",swapp," -freq ", freq, " -generations ",r ," -n 10 -recomb_rate ",rr ," -mutrate ",mr, " -seed ", seed, " -optimal_num 10", sep="")
	   	print(cmd)
	    system(cmd)
	}
}
...
for(i in 1:1){ #2
	...
	for(j in 1:1){ #3
		...
		for (k in 1:1){ #7
			rr<-rr_vec[k]
			name<-paste(N,"selection",swapp,"rr",rr,sep="")
			print(name)
