...
int check_plasticity(personal_records *personal_record){
    personal_gene_list *temp1=personal_record->ptr_to_new_genes_count;
    personal_gene_list *temp2=temp1->next;
        ...
        while (temp2->next!=NULL){
            for (i=0;i<genes_per_person;i++){
                if(temp1->indi_gene_array[i][1]==temp2->indi_gene_array[i][1]){
                    printf("Gene 1 %d: Gene 1a: %d\n",temp1->indi_gene_array[i][1],temp2->indi_gene_array[i][1]);
                    plastic_flag=1;
                }
            }
            if (plastic_flag) return 1;
            temp2=temp2->next;
        }
        temp1=temp1->next;
    }
    ...
    population_list *temp=plist_head;
        while(temp!=NULL){
            for(j=0;j<group_persons;j++){
               ...
               if(check_plasticity(temp->groups->personal_record[j])){
                    printf("plasticity in %d\n",j);
               }
            }
            temp=temp->next;
        }
