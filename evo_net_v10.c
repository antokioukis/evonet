...
typedef struct population_table{
    array_of_group* groups[population/group_persons];
}population_table;
...
personal_records *initialize_personal_records(int personal_id){
...
array_of_group *initialize_group_persons_array(){
    int i;
    ...
    for (i=0;i<group_persons;i++){
        ptr_grp_person_array->personal_record[i]=initialize_personal_records(i);
    }
    return ptr_grp_person_array;
}

population_table *initialize_population_table(){
    int i;
    population_table *ptr_pop_table;
    ptr_pop_table = (population_table *)malloc(sizeof(population_table));

    if(ptr_pop_table==NULL)
    {
        printf("Not enough memory for population\n");
        exit(-1);
    }

    for (i=0;i<population/group_persons;i++){
        ptr_pop_table->groups[i]=initialize_group_persons_array(i);
    }
    return ptr_pop_table;
}

void print_dump(population_table *p1){
    int i,j,k,l,m;
    for (m=0;m<population/group_persons;m++){
        for (i=0;i<group_persons;i++){
            printf("Gia to Atomo %d\n", p1->groups[m]->personal_record[i]->personal_id);
            for (j=0;j<genes_per_person;j++){
                printf("Gene No %d ",p1->groups[m]->personal_record[i]->ptr_to_genes_count->indi_gene_array[j][0]);
                printf("Gene Count %d ", p1->groups[m]->personal_record[i]->ptr_to_genes_count->indi_gene_array[j][1]);
                printf("R1 of Gene %s ", p1->groups[m]->personal_record[i]->ptr_to_personal_r1->personal_gene_r1[j]);
                printf("Balance of R1 %d ", p1->groups[m]->personal_record[i]->ptr_to_personal_r1->balance[j]);
                printf("R2 of Gene %s ", p1->groups[m]->personal_record[i]->ptr_to_personal_r2->personal_gene_r2[j]);
                printf("Balance of R2 %d ", p1->groups[m]->personal_record[i]->ptr_to_personal_r2->balance[j]);
                printf("\n");
            }
            ...
            for (k=0;k<genes_per_person;k++){
                for (l=0;l<genes_per_person;l++){
                    printf(" %f ",p1->groups[m]->personal_record[i]->gene_dependancies->gene_inter_matrix[k][l]);
                }
                printf("\n");
            }
            printf("\n");
...
int main(){
    population_table *p1;
    p1=initialize_population_table();
    print_dump(p1);
    return 0;
}
