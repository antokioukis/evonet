       ...
       /*"Advance the whole population*/
        while(temp!=NULL){
            for(j=0;j<group_persons;j++){
                create_new_gene_counts(temp->groups->personal_record[j]);
            }
            temp=temp->next;
        }
        temp=plist_head;
...
int check_plasticity(personal_gene_list *personal_recs){
    ...
    while(temp1!=NULL){
        while(temp2!=NULL){
            ...
            if (plastic_flag){return plastic_flag;}
            temp2=temp2->next;
        }
        temp1=temp1->next;
    }
...
void mature_pop(){
    aging(1);
    while(check_pop_plastic()!=1){
        total_gens++;
        aging(1);
        general_plasticity();
        printf("Generation %d\n",total_gens);
        if(total_gens>50000){
            printf("exit den tha teleiwsw pote genies panw apo 50000\n");
            exit(EXIT_FAILURE);
        }
    }
    printf("Xreiastika %d genies\n",total_gens);
}
